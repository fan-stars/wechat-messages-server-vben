...
import {
  createMessageForwardLog,
  getMessageForwardLog,
  updateMessageForwardLog,
...
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', ['转发日志'])
    : $t('ui.actionTitle.create', ['转发日志']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as MpMessageForwardLogApi.MessageForwardLog;
    try {
      await (formData.value?.id
        ? updateMessageForwardLog(data)
        : createMessageForwardLog(data));
      // 关闭并提示
